    ...
    <div class="card" style="margin-bottom: 5px">
      <el-table :data="data.tableData" stripe>
        ...
        <el-table-column label="订单状态" prop="status">
          <template #default="scope">
            <el-tag v-if="scope.row.status === '已退货'" type="danger">已退货</el-tag>
        ...
        <el-table-column label="操作"   width="160">
          <template #default="scope">
            <el-button v-if="data.user.role === 'USER' && scope.row.status === '待发货'" type="danger" @click="changeStatus(scope.row,'已退货')">退货</el-button>
...
const data = reactive({
  user: JSON.parse(localStorage.getItem('system-user') || '{"id":null,"role":""}'),
  pageNum: 1,
  pageSize: 10,
  total: 0,
  formVisible: false,
...
const load = () => {
  ...
  request.get('/orders/selectPage', {params}).then(res => {
    ...
    ElMessage.error('订单加载失败：' + (err.message || '网络异常'))
  })
}
