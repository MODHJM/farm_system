...
const data = reactive({
  ...
  rules:{
    username:[
      ...
      { pattern: /^[a-zA-Z]+$/, message: '账号只能包含英文字母', trigger: 'blur' }
    ]
  },
  tableData: [],
  name: null
...
const update = () => {
  request.put('/admin/update', data.form).then(res => {
    if (res.code === '200') {
      ...
      if (data.user.id && data.form.id === data.user.id) {
        // 合并旧用户信息和新编辑的信息
        const newUser = { ...data.user, ...data.form }
        localStorage.setItem('system-user', JSON.stringify(newUser))
        emit('updateUser') // 只有编辑自己时，才通知布局页刷新
      }
